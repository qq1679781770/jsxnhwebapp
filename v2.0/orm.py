...
# -*- coding:utf8 -*-
# Data:2017-08-23
# Author:jsxnh(qq1679781770@gamil.com)
# @version:0.1
...
def select(sql, args, size=None):
    log(sql, args)
    cf = init_config(**config.db_config)
...
def execute(sql, args, autocommit=True):
    ...
    try:
        with conn.cursor() as cur:
            affected = cur.execute(sql.replace('?', '%s'), args or ())
        conn.commit()
        conn.close()
    except:
        conn.close()
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    @classmethod
    def count(cls):
        sql = "select count(*) num from `%s`" % cls.__table__
        count = select(sql, args=None)
        return count[0]['num']
